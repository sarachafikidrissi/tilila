...
export default function AdminNewsForm({ article }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        if (isEdit) {
            post(`/admin/program/news/${article.id}`, { forceFormData: true, _method: 'put' });
        } else {
    ...
    return (
        <AppLayout>
            ...
            <form onSubmit={submit} className="mx-auto max-w-2xl space-y-4 p-6">
                <h1 className="text-2xl font-bold">{isEdit ? 'Edit' : 'New'} article</h1>
                <div>
                    <Label>Program</Label>
                    <select className="w-full rounded-md border px-3 py-2" value={data.program ?? ''} onChange={(e) => setData('program', e.target.value || null)}>
                ...
                <div>
                    ...
                    <Input value={data.title.fr} onChange={(e) => setData('title', { ...data.title, fr: e.target.value })} />
                </div>
                <div>
                    <Label>Slug</Label>
                ...
                <div>
                    ...
                    <textarea className="w-full rounded-md border px-3 py-2" rows={2} value={data.excerpt.fr} onChange={(e) => setData('excerpt', { ...data.excerpt, fr: e.target.value })} />
                </div>
                <div>
                    <Label>Body (FR)</Label>
                    <textarea className="w-full rounded-md border px-3 py-2" rows={6} value={data.body.fr} onChange={(e) => setData('body', { ...data.body, fr: e.target.value })} />
                </div>
                <div>
                ...
                <div>
                    ...
                    <Input type="file" accept="image/*" onChange={(e) => setData('cover_image', e.target.files[0])} />
                </div>
