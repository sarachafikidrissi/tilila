        ...
        <>
            ...
            <div className="mx-auto max-w-3xl px-4 py-10">
                ...
                <h1 className="mt-4 text-2xl font-bold text-tblack">
                    <TransText en="Apply to Tililab" fr="Déposer une candidature Tililab" ar="الترشح لتيليلاب" />
                </h1>
                ...
                <form onSubmit={submit} className="mt-8 space-y-8">
                    <fieldset className="space-y-4 rounded-2xl border border-border p-6">
                        ...
                        <Field label="Date de naissance" error={errors.birth_date}>
                            <input type="date" className={inputClass} value={data.birth_date} onChange={(e) => setData('birth_date', e.target.value)} required />
                        </Field>
                        <Field label="CIN" error={errors.cin}>
                            <input className={inputClass} value={data.cin} onChange={(e) => setData('cin', e.target.value)} required />
                        </Field>
                        <div className="grid gap-4 sm:grid-cols-2">
                            <Field label="Ville" error={errors.city}>
                                <input className={inputClass} value={data.city} onChange={(e) => setData('city', e.target.value)} required />
                            </Field>
                        ...
                        <Field label="Niveau d'études" error={errors.education_level}>
                            <input className={inputClass} value={data.education_level} onChange={(e) => setData('education_level', e.target.value)} required />
                        </Field>
                        <Field label="Profession" error={errors.profession}>
                            <input className={inputClass} value={data.profession} onChange={(e) => setData('profession', e.target.value)} required />
                        </Field>
                        <Field label="LinkedIn / Instagram / Portfolio" error={errors.social_links}>
                            <input className={inputClass} value={data.social_links} onChange={(e) => setData('social_links', e.target.value)} />
                        </Field>
                    </fieldset>
                    ...
                    <fieldset className="space-y-4 rounded-2xl border border-border p-6">
                        ...
                        <Field label="Titre du projet" error={errors.project_title}>
                            <input className={inputClass} value={data.project_title} onChange={(e) => setData('project_title', e.target.value)} required />
                        </Field>
                        <Field label="Lien vers une réalisation antérieure" error={errors.prior_work_link}>
                            <input type="url" className={inputClass} value={data.prior_work_link} onChange={(e) => setData('prior_work_link', e.target.value)} />
                        </Field>
                        <Field label="Upload Portfolio" error={errors.portfolio_file}>
                            <input type="file" accept=".pdf,.zip" onChange={(e) => setData('portfolio_file', e.target.files[0])} />
                        </Field>
                    ...
                    <fieldset className="space-y-4 rounded-2xl border border-border p-6">
                        <Field label="Présentation du candidat" error={errors.candidate_presentation}>
                            <textarea rows={4} className={inputClass} value={data.candidate_presentation} onChange={(e) => setData('candidate_presentation', e.target.value)} required />
                        </Field>
                        <Field label="Présentation du projet" error={errors.project_presentation}>
                            <textarea rows={4} className={inputClass} value={data.project_presentation} onChange={(e) => setData('project_presentation', e.target.value)} required />
                        </Field>
                        <Field label="Message principal" error={errors.main_message}>
                            <textarea rows={3} className={inputClass} value={data.main_message} onChange={(e) => setData('main_message', e.target.value)} required />
                        </Field>
                        <Field label="Motivation" error={errors.motivation}>
                            <textarea rows={3} className={inputClass} value={data.motivation} onChange={(e) => setData('motivation', e.target.value)} required />
                        </Field>
                    </fieldset>

                    <fieldset className="space-y-3 rounded-2xl border border-border p-6">
                        <CheckRow id="u30" checked={data.declared_under_30} onChange={(v) => setData('declared_under_30', v)} label="Je certifie être âgé(e) de moins de 30 ans" error={errors.declared_under_30} />
                        <CheckRow id="acc" checked={data.declared_accuracy} onChange={(v) => setData('declared_accuracy', v)} label="Je certifie l'exactitude des informations" error={errors.declared_accuracy} />
                        <CheckRow id="rights" checked={data.declared_rights} onChange={(v) => setData('declared_rights', v)} label="Je suis titulaire des droits relatifs aux éléments transmis" error={errors.declared_rights} />
    ...
    return (
        <div>
            <div className="flex items-center gap-2">
                ...
                <Label htmlFor={id} className="text-sm text-tgray">{label}</Label>
            </div>
            {error ? <p className="mt-1 text-xs text-red-600">{error}</p> : null}
        </div>
    );
